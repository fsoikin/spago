...
discoverWorkspace options cwd = do
  ...
  { workspace, rootPath } /\ { loadedPackages } <-
    State.runStateT (walkDirectoriesUpFrom cwd) { loadedPackages: Map.empty, otherWorkspaceRoots: [], misnamedConfigs: [] }
  ...
  selected <-
    determineSelectedPackage options.maybeSelectedPackage packagesByName
...
determineSelectedPackage :: ∀ a. Maybe PackageName -> Map PackageName WorkspacePackage -> Spago (Registry.RegistryEnv a) (Maybe WorkspacePackage)
determineSelectedPackage requestedName loadedPackages = do
  -- Select the package for spago to handle during the rest of the execution
  maybeSelected <- case requestedName of
